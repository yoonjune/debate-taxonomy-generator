...
- **Content** is binary per code, judged by an LLM from the utterance text: A4 says "ten seconds"; A2-2 hands over (name / other side / next); A3-1 announces the move to the next phase (any wording) **and** the length; A3-2 says time is up **and** moves to closings; A1 / A2-1 stop the speaker for time; A5 restrains the interrupter; B1 points out the drift and steers back; B2 points out the self-contradiction **and** asks the speaker to reconcile it. Pass requires every criterion; the judge also records `predicted_label` (which action the utterance performed). Joint = ON_TIME ∧ pass. Filler-only utterances (mm-hm, yeah, okay) or utterances under 0.4 s with at most one word are back-channels and never attributed; one-word moderator lines such as "Time." count.
