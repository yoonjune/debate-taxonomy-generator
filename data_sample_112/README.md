# data_sample_112 — 토론 진행자 벤치마크 112편
...
```
토론 112편 · 발화 3514개 · trigger 1071개 · 크로스파이어 2:30 고정
```
...
필수는 매 편, 선택은 편당 2~3개를 균등하게 넣었다 (A1 59 · A2-1 58 · A5 53 · B1 57 · B2 64). `A1`과 `A2-1`은 같은 30초 컷이고,
라운드의 첫 화자(PRO)면 넘겨야 하므로 `A2-1`, 마지막 화자(CON)면 부를 사람이 없어 `A1`이다.

### 끊기는 자리의 소리 길이

채점 창은 언제나 마감 +2초까지다. 10초 고지는 마감 20초 · 창 끝 **22초**, 시간 초과 중단은
마감 30초 · 창 끝 **32초**다. 이 코드들은 "화자가 아직 말하고 있으니 끊어라"가 전제라,
창이 끝날 때까지 화자 목소리가 들려야 한다. 그래서 A1 · A2-1 · A3-2 · 크로스파이어 A4 의
발화는 창 끝을 1.5초 넘겨서까지 이어진다. 화자가 제 시간에 끝낸 A2-2 · A3-1 · B1 · B2 는
반대로 그 자리가 침묵이다 — 그게 정답이다.
...
```
오디오는 합성 완료본이다 (OmniVoice 음성 복제, 24 kHz 모노, mp3 64 kbps). `audio/mix/<id>.json`의 시각은 실제 합성 오디오의 시각이고, 탐침 창도 그 시각으로 확정했다. 발화 파일 3514개, 전체 길이 570분 (편당 4.6~5.5분, 중앙 5.1분).
