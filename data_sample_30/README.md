# data_sample_30 — 토론 진행자 벤치마크 30편
...
```
토론 30편 · 발화 969개 · trigger 286개 · 크로스파이어 2:30 고정
```
...
필수는 매 편, 선택은 편당 2~3개를 균등하게 넣었다 (A1 17 · A2-1 16 · A5 17 · B1 16 · B2 16). `A1`과 `A2-1`은 같은 30초 컷이고,
...
```
오디오는 합성 완료본이다 (OmniVoice 음성 복제, 24 kHz 모노, mp3 64 kbps). `audio/mix/<id>.json`의 시각은 실제 합성 오디오의 시각이고, 탐침 창도 그 시각으로 확정했다. 발화 파일 969개, 전체 길이 146분.
